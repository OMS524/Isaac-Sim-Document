...
RViz2와 같은 많은 ROS 2 노드는 매개변수 `use_sim_time`을 사용하며,
...
현재 실행 중인 노드로 `/node_name`을 교체해야 합니다.
...
- **Isaac Read Simulation Time**: 현재 시뮬레이션 시간을 가져옵니다. 참고: 기본적으로 시뮬레이션 시간은 단조롭게 증가하므로 시뮬레이션이 중지되었다가 다시 재생되더라도 시간은 계속 증가합니다. 이는 주로 시뮬레이션이 초기화될 때 시간이 뒤로 이동할 때 발생할 수 있는 문제를 방지하기 위한 것입니다. 시뮬레이션이 초기화될 때마다 시계를 0에서 시작하도록 하려면 `resetOnStop`을 True로 설정할 수 있습니다.
...
- **On Playback Tick**: 모든 시뮬레이션 프레임에서 다른 그래프 노드를 실행합니다.
- **ROS 2 Context**: 주어진 도메인 ID 또는 `ROS_DOMAIN_ID` 환경 변수를 사용하여 컨텍스트를 생성합니다.
- **ROS2 Subscribe Clock**: 외부 타임스탬프 데이터를 Subscribe 합니다.

3. Play를 클릭하여 시뮬레이션을 시작합니다. Action graph 내에서 ROS2 구독 시계 노드를 선택하여 Property 창에서 `timeStamp` 출력을 확인합니다. 타임스탬프가 0인지 확인합니다.
