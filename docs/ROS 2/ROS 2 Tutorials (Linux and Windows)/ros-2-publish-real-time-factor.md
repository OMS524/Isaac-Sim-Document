...
## Publish RTF
RTF는 시뮬레이션이 실시간과 비교하여 얼마나 빠르거나 느린지를 나타냅니다. 이는 프레임당 `RTF = simulated_elapsed_time / real_elapsed_time`으로 계산됩니다. RTF가 1보다 크면 시뮬레이션 시간이 wall clock time보다 빠르게 실행됩니다. RTF가 1보다 작으면 시뮬레이션이 실시간보다 느리게 실행됩니다.
